...
engine = create_engine(
    'mysql+mysqlconnector://root:123Bnm890#!@localhost:3306/books',
    echo=True)
...
def add_book(book: Book, author: Author):
    with Session(engine) as session:
        ...
        existing_author = session.execute(select(BookAuthor).filter(Author.first_name == author.first_name, Author.last_name == author.last_name)).scalar()
        ...
        session.add(pairing)
        session.commit()
        print("New book added " + book.title + " " + str(book.number_of_pages) + " New author added " + author.first_name + " " + author.last_name)
...
def get_book_from_db(book_id):
    with Session(engine) as session:
        ...
        book_info = session.execute(select(BookAuthor).filter(Book.book_id == book_id)).scalar()
        author = session.execute(select(Author).filter(Author.author_id == book_info.author_id )).scalar()
        return (book, author)